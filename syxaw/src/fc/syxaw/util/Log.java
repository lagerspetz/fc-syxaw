...
// $Id: Log.java,v 1.31 2005/01/04 15:11:39 ctl Exp $
...
public class Log {
  ...
  static {
    if( System.getProperty("syxaw.log.file.really",null)!= null ) {
  ...
  public static void log( String msg, int level, Object o, int stpos ) {    
    ...
    if(true) {
      // Redirect to Fuego logging infra!
      fc.util.log.Log.log(msg, level, o);
      return;
    }
    try {
    if( firstTs == -1 ) {
      firstTs = System.currentTimeMillis();
      log("Time zero, showing up to level "+LEVEL,Log.INFO, new Date(firstTs) );
    }
    ...
    if (level <= FATALERROR) {
      ...
      System.exit( -1);
    	...
    	lout.println("<<LOG INTERNAL ERROR>> "+e.getClass()+": "+ e.getMessage());
