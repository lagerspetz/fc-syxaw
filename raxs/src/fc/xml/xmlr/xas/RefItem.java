...
import fc.xml.xas.AppendableItem;
...
/** XAS Reference item. This class provides a bridge between 
 * {@link fc.xml.xmlr.Reference References} and XAS items. 
 * 
 */
//BUGFIX-20070216-1: Serialization fixes: 1) emit <ref:tree> end tag,
// 2) use context to omit unnecessary prefix mapping in <ref:node>,
// 3) implement AppendableItem instead of SerializableItem (per purpose
// of the interfaces)
abstract public class RefItem extends Item implements AppendableItem {
...
//  public static final String REF_ATT_TARGET = "target";
  ...
  public static final int whatItem(Item i)  {
    ...
    if( i.getType() == Item.START_TAG ) {
      if(REF_TAG_TREE.equals(((StartTag) i).getName() ) )
        return ITEM_TREEREF_OPEN;
      else if ( REF_TAG_NODE.equals(((StartTag) i).getName() ) ) 
        return ITEM_NODEREF_OPEN;
      else
        return ITEM_NONE;
    }
    if( i.getType() == Item.END_TAG ) {
      if( REF_TAG_NODE.equals(((EndTag) i ).getName() ) ) 
        return ITEM_NODEREF_CLOSE;
      else if( REF_TAG_TREE.equals(((EndTag) i ).getName() ) ) 
        return ITEM_TREEREF_CLOSE;
      else
        return ITEM_NONE;
    }        
    if( (i.getType() & CATEGORY_MASK) == CATEGORY )
      return ITEM_DECODED; // Already decoded
    return ITEM_NONE;
  }

  /** Decode XAS item as reference item. 
   * 
   * @param i item to decode. 
   * @return decode RefItem, or <i>i</i> if <i>i</i> is already a RefItem
   * @throws IOException if an I/O error occurs
   */
  public static final Item decode(Item i) throws IOException {
    switch(whatItem(i)) {
      case ITEM_DECODED:
      case ITEM_TREEREF_CLOSE: // null is perhapse better?
      case ITEM_NONE: return i;
      case ITEM_TREEREF_OPEN: return new RefTreeItem((StartTag) i,
            ((StartTag) i).getContext());
      case ITEM_NODEREF_OPEN: return new RefNodeItem((StartTag) i,
          ((StartTag) i).getContext());
      case ITEM_NODEREF_CLOSE: return new RefNodeItem((EndTag) i);
      default: //A! assert false;
      }
    //A! assert false;
    return null;
  }
  
  /** Check if item decodes as ref item. An instance of <code>RefItem</code>
   * does <i>not</i> decode as a reference item. 
   * 
   ...
   * 
   * @param r reference to encode
   * @param ctx XAS context
   * @return start item
   */
  public static RefItem makeStartItem( Reference r, StartTag ctx ) {
    ...
    if( r.isTreeReference() )
      return new RefTreeItem(r.getTarget(),ctx);
    else
      return new RefNodeItem(r.getTarget(),ctx);
  }
  ...
  public abstract Iterator /*!5 <AttributeNode> */ getExtraAttributes();
