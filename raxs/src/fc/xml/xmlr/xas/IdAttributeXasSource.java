...
/** RandomAccessSource that is navigated by keys read from the "id"
 * attribute.
 */
public abstract class IdAttributeXasSource implements RandomAccessSource 
  /*!5 <Key> */ {
  ...
  /** Create new source. The corresponding key identification model
   * is {@link KeyIdentificationModel#ID_AS_STRINGKEY}.
   * 
   * @param xd Xas Document
   * @param root root id
   */
   ...
   * @param xd Xas Document
   * @param root root id
   * @param kim key identification model
   */
  ...
  public static GlobalPointer getRootPointer(Document xd) 
    throws MalformedXml {
  ...
  public Iterator /*!5 <Key> */ getChildKeys(Key k) throws NodeNotFoundException {
    ...
    if( p == null )
      throw new NodeNotFoundException();
    final Iterator /*!5 <GlobalPointer> */ cpi = p.childPointers();
    return new Iterator /*!5 <Key> */() {
      ...
      public Object next() {
        try {
          return identify((GlobalPointer) cpi.next());
        } catch (IOException e) {
          trap(e);
        }
        //A! assert false;
  ...
  public Item next() throws IOException {
    return (Item) (ensureItemIterator().hasNext() ? ii.next() : null);
  }
  ...
  protected void trap(IOException e) {
    if( trap == null )
      throw new IOExceptionTrap.RuntimeIOException(e);
    else
      trap.trap(e);
  }
  ...
  /** Convenience implementation that builds a hashmap index of all ids
   * in memory.
   */
  public static class MemoryIdIndexSource extends IdAttributeXasSource {

    protected static Map /*!5 <Key,GlobalPointer> */ index;
    ...
    protected GlobalPointer lookup(Key id) {
      return (GlobalPointer) index.get(id);
    }

    protected void initLookup(Document xd) {
      index = new HashMap /*!5 <Key,GlobalPointer> */();
      try {
        doInit(xd.getRoot());
      } catch (IOException e) {
        trap(e);
    ...
    protected void doInit(GlobalPointer p) throws IOException {
      ...
      for( Iterator /*!5 <GlobalPointer> */ i = p.childPointers();i.hasNext();)
        doInit((GlobalPointer) i.next());
    }
