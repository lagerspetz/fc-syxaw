...
/** Reftree node keyed by Dewey keys. Nodes know their position in
 * the parent's child list, so keys are very fast to compute.
 * Child lists are implemented using {@link java.util.ArrayList}, with
 * the performance implications of that list.
 */
public class DeweyRefNode implements RefTreeNode {
  ...
  private List /*!5 <DeweyRefNode> */ children = null;
   ...
   * @param childPos position in child list of parent
   * @param content node content. 
   */
  
  public DeweyRefNode(DeweyRefNode parent, int childPos, Object content ) {
    //A! assert childPos >= 0;
    //A! assert parent != null : "The root should have a PrefixNode, not null!";
    this.content = content;
    this.parent = parent;
  ...
  public Key getId() {
    ...
    for(;prefix.parent!=null;prefix=prefix.parent)
      depth++;
    //A! assert prefix instanceof DeweyRefNode.PrefixNode : 
    //A!  "Expected a prefix here, got "+prefix.getClass()+
    //A!  "childPos is "+childPos;
    int[] key = new int[depth];
    // Below: p.parent is a pathological case (detached node)
    for(DeweyRefNode p=this;p.parent != null && 
      p.parent.parent!=null;p=p.parent)
      key[--depth]=p.childPos;
    return ((DeweyKey) prefix.getId()).append(key);
  }

  public RefTreeNode getParent() {
    // BUGFIX-20071109-1: Parent of root is null, not the prefix node
    return parent.parent == null ? null : parent;
  }
  ...
  protected List /*!5 <DeweyRefNode> */ ensureChildren() {
    if( children == null )
      children = new ArrayList /*!5 <DeweyRefNode> */();
    return children;
  }
  ...
  public RefTreeNode get(int i) {
    return (RefTreeNode) ensureChildren().get(i);
  }
  
  public void addChild(DeweyRefNode child) {
    //A! assert child != null;
    //A! assert child != this; // Circular relationship!
  ...
  public void addChild(int ix, DeweyRefNode child) {
    ...
    for( int i = ix + 1;i<children.size();i++)
      ((DeweyRefNode) children.get(i)).childPos=i;
    child.childPos=ix;
    child.parent=this;
  }

  public boolean removeChild(DeweyRefNode child) {
    //A! assert ensureChildren().get(child.childPos) == child;
  ...
  void detach() {
    if( parent != null ) {
      // BUGFIX-071112-2: Fixed detach/attach of the root
      if ( !(parent instanceof PrefixNode) ) {
  ...
  void attach(DeweyRefNode parent) {
    if( parent != null ) {
      ...
      //A! assert false : "Cannot attach to null";
      //parent = null;
      //childPos=0;
    }
  }
  ...
  /** Node model for this class, with root keyed as <code>/0</code>. In many
   * cases (XML documents with document root as tree root), it is useful to 
   * key the root as <code>/0</code>.
   */
  ...
  private static class DeweyNodeModel implements NodeModel {
    ...
    public RefTreeNode build(RefTreeNode parent, Key key, Object content, 
        int pos) {
      DeweyRefNode dp = (DeweyRefNode) parent;
      DeweyRefNode n = new DeweyRefNode(
          dp!=null ? dp :
            new DeweyRefNode.PrefixNode(pk), 0 , content);
      if( dp != null ) {
