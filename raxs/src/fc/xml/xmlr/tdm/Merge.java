 ...
 * nodes that need to be visited during the merge are expanded.
 */
...
public class Merge {
  ...
  protected Map /*!5 <Key,RefTreeNode> */ baseIx = 
      new HashMap /*!5 <Key,RefTreeNode> */();
  protected Map /*!5 <Key,BranchNode> */ leftIx = 
      new HashMap /*!5 <Key,BranchNode> */();
  protected Map /*!5 <Key,BranchNode> */ rightIx = 
      new HashMap /*!5 <Key,BranchNode> */();
  ...
  public static void merge( IdAddressableRefTree base, IdAddressableRefTree t1,
                            IdAddressableRefTree t2, ItemTarget tm,
                            ItemTarget log,
                            XasCodec cw,
                            NodeMerger nodeMerger) throws IOException {
    Merge m = new Merge(base,t1,t2,null,null,nodeMerger);
    /*{ // DEBUG
      Log.info("============MERGE========");
      m.merge( new ItemList(), new XasExternalizer(XasDebug.itemDump(),cw));
      Log.info("============EOM========");
    }*/
  ...
  protected Merge( IdAddressableRefTree base, IdAddressableRefTree t1,
   ...
   /* This is not necessary with uniqueKey
   ...
   // BUGFIX-20061017-2: t1r was assigned twice->probably busted merge of root
   t2r = new BranchNode(fakeRoot, t2.getRoot(), baseIx, false);
   this.nm = nm;
  }

  protected void merge( ItemTarget log, tdm.lib.XMLNode.Externalizer ext )
      throws IOException {
    try {
  ...
  public static class XasExternalizer implements tdm.lib.XMLNode.Externalizer {
    ...
    public void startNode(tdm.lib.XMLNode xn) throws IOException {
      ...
      if( n.isReference() )
        wr.append(RefItem.makeStartItem(n.getReference(),wr.getContext()));
      else {
        wr.mark();
    ...
    public void endNode(tdm.lib.XMLNode xn) throws IOException {
      ...
      if( n.isReference() ) {
        if( n.isNodeRef() )
          wr.append(RefItem.makeEndItem(n.getReference()));
      } else 
        wr.close();
    }
    ...
    private static class TagCloser implements ItemTarget {
      ...
      Stack /*!5 <Item> */ closing=new Stack /*!5 <Item> */();
      ...
      public void close() throws IOException {
       for( Item i=null;!closing.isEmpty() && (i=(Item) closing.pop())!=null;) {
         if( RefItem.isRefItem(i) )
           continue;
         StartTag st = (StartTag) i;
         if( st.getName() == MARKER )
           break;
         t.append( new EndTag( st.getName() ) );
       }
       Item ci = !closing.isEmpty() ? (Item) closing.peek() : null;
       context = ci == null || Item.isStartTag(ci) ?
           (StartTag) ci : ((RefItem) ci).getContext();
      }
      
      public void flush() throws IOException {
         if( t instanceof SerializerTarget )
          ((SerializerTarget) t).flush(); 
      }
  ...
  class BranchNode extends tdm.lib.BranchNode {
    ...
    public void debugTree(PrintWriter pw, int indent) {
      /*pw.println("                ".substring(15-indent)
          +n+", "+n.getContent()+", basem="+getBaseMatch());
