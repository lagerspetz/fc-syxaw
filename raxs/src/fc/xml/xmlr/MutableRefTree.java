 ...
 * <p>The interface accommodates for ordered as well as
...
public interface MutableRefTree extends IdAddressableRefTree {
  ...
  /** Constant indicating that an automatic key assignment is desired. Used
   * with {@link #insert(Key, long, Key, Object)}. The value of this field is known to be 
   * <code>null</code>, i.e., the field name is just to increase readability of
   * the code.
   */
   ...
   */
  public Key insert(Key parentId, Key newId,
   ...
   * {@link #DEFAULT_POSITION DEFAULT_POSITION}. The 0-based target 
   * position denotes the position in a child list from which the node has been
   * removed. I.e.,
   * if the node <i>p</i> has the child list <i>a,b,c</i>, then 
   * <code>move(<i>b</i>,<i>p</i>,1)</code> will yield the child list 
   * <i>b,a,c</i>, and not <i>a,b,c</i>.
   * <p><b>Note:</b> The result of a move
   * that would make a node an ancestor of itself is undefined. Some
   * implementations may detect this condition and throw an
   * {@link java.lang.IllegalArgumentException}
   ...
   *
   * @param nodeId id of node whose content is updated
