...
public class RandomDirectoryTree {
  ...
  public static void permutateTreeStringOrder(MutableRefTree t,
      long ops, String pdf, double
      deleteTreeProb, final Random rnd,
      KeyGen kg ) {
    permutateTree(t,ops, pdf, deleteTreeProb, rnd, kg,
        new IdNodePicker(
        new RandomizedSetByString(rnd.nextInt()),t)); 
  }  

  public static void permutateTree(MutableRefTree t,
      long ops, String pdf, double
      deleteTreeProb, final Random rnd,
      KeyGen kg) {
    permutateTree(t,ops, pdf, deleteTreeProb, rnd, kg,
        new SafeIdNodePicker(t,rnd));
  }
  
  /** Permutate a reftree by random edits. The edits and their symbols are
   * insert leaf (<code>i</code>), insert directory (<code>I</code>),
   * delete leaf (<code>d</code>), update node (<code>u</code>), and
   * move node (<code>m</code>). 
   * 
   * @param t Tree to edit
   * @param ops number of edits
   * @param pdf relative frequency of edits. Each type of edit has a symbol,
   * and the probability of the edit type is the relative frequency of that
   * symbol in this string. E.g. "<code>mmmu</code>" = 75% moves, 25% updates 
   * @param deleteTreeProb probability that a leaf delete deletes is parent
   * (recursively applied towards root)
   * @param rnd random generator
   * @param kg id generator for inserted nodes 
   * @param np randomized set of editable nodes. Should be initialized to
   * the nodes allowed to edited in t.
   */
  
  public static void permutateTree(MutableRefTree t,
                                   long ops, String pdf, double
                                   deleteTreeProb, final Random rnd,
                                   KeyGen kg, NodePicker np) {
    int redoFails=0;
    ...
    for(;ops>0;ops--) {
      ...
      redo = false;
      // The selection procedure below may choose a file only once, but a dir as
      // many times as it has files. Also, the probability of a dir is
      // weighted by the numbers of files in it
      RefTreeNode rndNode =  pos;
      //A! assert rndNode != null : "Node disappeared "+rndNode.getId();
      RefTreeNode rndDirNode = rndNode;
      int dscan = np.size();
      //A! assert rndDirNode.getContent() != null;
      while (
        ...
        rndDirNode = pos;
        if( rndDirNode == null ) {
          Log.fatal("Tried to select non-existing node "+pos.getId(),
              new Throwable());
          redo = true;
          break top;
        }
      ...
      try {
        ...
        switch (op) {
          case 'i': { // File ins
            ...
            Object nc = new DirectoryEntry(nId, fileName(nId,rnd),
                DirectoryEntry.FILE);
            Key nKey =
              t.insert(rndDirNode.getId(), iKey, nc );
            //Log.log("ins-f @ "+rndDirNode.getId()+" ",Log.INFO, t.getNode(nId).getContent());
            if( edstring != null )
              edstring+="i("+preDirNodeId +","+nc+")\n";
            np.addNode(t.getNode(nKey), false); // treeNodes.add(nId);
            break;
          }
          case 'I': {  // Dir ins
            Key iKey = makeInsertId(t,nId);
            Object nc = new DirectoryEntry(nId, dirName(nId,rnd),
                DirectoryEntry.DIR);
            Key nKey =
              t.insert(rndDirNode.getId(), iKey, nc);
...
//              rndNode = rndNode.firstChild();
        ...
        Log.log("Selected nonexisting node "+ex.getId()+", edstring so far="
            +edstring,Log.FATALERROR,ex);
      }
    } // End for
  }
  ...
  protected static Key makeInsertId(IdAddressableRefTree t, Key kg ) {
    if( useInsertAutoKey )
      return fc.xml.xmlr.MutableRefTree.AUTO_KEY;
    return kg;
  }
  ...
  public static class SafeIdNodePicker implements NodePicker {
    ...
    ArrayList /*!5 <Key> */ queue = new ArrayList /*!5 <Key> */();
    int length;
    Random rnd;
    int pos=0;
    ...
    public SafeIdNodePicker( IdAddressableRefTree target, Random rnd, Key root ) {
      ...
      if(_verbose)
        Log.debug("Queue after init",queue.subList(0, length));
    }
    ...
    private void remove( Set /*!5 <Key> */ s ) {
      int initLen = length;
      //A! assert !s.contains(null);
      if(_verbose)
        Log.debug("Set before remove of "+s,queue.subList(0, length));
      for(int i=0;i<length;i++) {
        if( s.contains( queue.get(i) ) ) {
          if( length > 1 ) {
            queue.set(i, queue.get(--length));
            i--; // NOTE: It may be that the one we swapped at the delpos should be deleted!
            if(queue.set(length, null)==null ) {
              Log.warning("Queue is",queue.subList(0, length));
              Log.warning("Deletia is",s);
            }
            ////A! assert queue.set(length, null)!=null; // Sets unused to null
          } else { 
            queue.clear();
            length=0;
          }
      ...
      if(_verbose)
        Log.debug("Set after remove",queue.subList(0, length));
      //A! assert length == initLen - s.size();

      ////A! assert (new HashSet(queue.subList(0, length))).size() == length;
    }
    
    public RefTreeNode nextNode() {
      if(_verbose)
         Log.debug("Picking @ "+(pos+1),queue.get( ((pos+1)%length)));
      return target.getNode( (Key) queue.get( ((++pos)%length) ) );
    }
    ...
    public void removeNode(RefTreeNode root, boolean recurse ) {
      Set /*!5 <Key> */ s = new HashSet  /*!5 <Key> */();
      removeNode(root,recurse,s);
      remove(s);
    }
    
    private void removeNode(RefTreeNode root, boolean recurse, 
        Set /*!5 <Key> */ s) {
      s.add(root.getId());
      if( !recurse )
        return;
  ...
  public static class IdNodePicker implements NodePicker {
    ...
    protected Key nextKey() {
      try {
        SortedSet /*!5 <Key> */ tmp = s.subSet(s.first(), pos);
        //Log.log("tmp="+tmp,Log.INFO);
        Key k = (Key) (tmp.size() == 0 ?
            ( s.size() > 0 ? s.last() : null ): tmp.last());
            return k;   
  ...
  public static class TransientNodePicker implements NodePicker {
    ...
    public RefTreeNode nextNode() {
      ...
      ////A! assert false;
      return size > 0 ? nextNode() : null; // Retry... (Mental note: this is going to bite me
  ...
  public static MutableDirectoryTree randomDirTree(long nodes, long nodesPerDir, double dirProb,
    ...
    MutableDirectoryTree t = new MutableDirectoryTree(id,
           new DirectoryEntry(id, null,DirectoryEntry.TREE));
    try {
      LinkedList /*!5 <Key> */ roots = new LinkedList /*!5 <Key> */();
      roots.add(t.getRoot().getId());
      while (nodes > 0 && roots.size() > 0) {
        Key parentId = (Key) roots.removeFirst();
  ...
  public static String fileName(Key id, Random rnd) {
    ...
    return fnames[rnd.nextInt(fnames.length)]+"-"+id+"."+
        exts[rnd.nextInt(exts.length)]; //+rnd.nextInt();
  }
  ...
  public static RefTree treeCopy(RefTree t) {
    ...
    //A! assert XmlrDebug.treeComp(t,copy) : "treeCopy broken";
    return copy;
  }

  public static RefTreeNodeImpl treeCopy(RefTreeNode root, RefTreeNode parent) {
    RefTreeNodeImpl newRoot
        = new RefTreeNodeImpl( parent,root.getId(),root.isTreeRef(),null);
  ...
  private Map /*!5 <Key,RefTreeNode> */ index = 
    new HashMap /*!5 <Key,RefTreeNode> */();
  ...
  public void delete(Key id) throws NodeNotFoundException {
    ...
    if( p == null )
      throw new IllegalArgumentException("Trying to delete root");
  ...
  private int getTargetPos(RefTreeNode parent, RefTreeNode n) {
    ...
    for( Iterator i = parent.getChildIterator();i.hasNext();) {
      if( ID_BY_ORDER.compare(n, i.next()) > 0 )
        pos ++;
      else
        return pos;
    }
