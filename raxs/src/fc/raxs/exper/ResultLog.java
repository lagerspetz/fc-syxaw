...
public class ResultLog {
  ...
  public ResultLog(String test, String store, String by, Class[] fieldClasses,
    ...
    char SEP = ' ';
    ArrayList /*!5 <Object> */ markers = new ArrayList /*!5 <Object> */();
    ArrayList /*!5 <Field> */ fields = new ArrayList /*!5 <Field> */();
    units = null;
    try {
      for( int ip=0;ip<points.length;ip++ ) {
        Object point = points[ip];
        for( int ic=0; ic<fieldClasses.length; ic++ ) {
          Class c = fieldClasses[ic];
          Field[] fielda = c.getFields(); 
          for( int ifi = 0; ifi < fielda.length; ifi++  ) {
            Field f = fielda[ifi];
            if( f.get(null) == point ) {
              fields.add(f);
              markers.add(point);
    ...
    this.fields = (Field[]) fields.toArray(new Field[] {});
    this.test = test;
    this.store = store;
    this.by = by;
    this.out = out;
