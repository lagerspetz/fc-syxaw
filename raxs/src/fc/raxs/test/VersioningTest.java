...
public class VersioningTest extends TestCase {
  ...
  public void testIdVersioning() throws IOException {
    ...
    KeyGen kg = new KeyGen( RandomDirectoryTree.KEY_GEN );
    ...
    LinkedList /*!5 <RefTree> */ facits=  new LinkedList /*!5 <RefTree> */();
    facits=null; // Disable this to enable test self-check
    LinkedList /*!5 <Integer> */ vlist= new LinkedList /*!5 <Integer> */();
    vlist.add( new Integer( raxs.commit(raxs.getEditableTree()) ) ); // Initial ver
    if( facits != null )
    ...
    kg = new KeyGen( RandomDirectoryTree.KEY_GEN );
    dt = RandomDirectoryTree.randomDirTree(TSIZE, DSIZE, DPROB, VAR, DVAR, new Random(RND_SEED),kg);
    ((RandomDirectoryTree.MutableDirectoryTree ) dt).setOrdered(true);

    for( int lap = -1;lap < MAX_LAP;lap++) {
      int version = ((Integer) vlist.removeFirst()).intValue();
      Debug.Time.stamp();
      RefTree ot = raxs.getTree(version);
      Log.info("Retrieved "+ version+" in "+Debug.Time.sinceFmt());
      if( facits != null) {
        RefTree ft = (RefTree)facits.removeFirst();
    ...
    //!FPD System.runFinalizersOnExit(true);
  }
  ...
  public void testDeweyVersioning() throws IOException, NodeNotFoundException {
    ...
    Random oprnd = new Random(RND_SEED);
    KeyGen kg = new KeyGen( RandomDirectoryTree.KEY_GEN );
    MutableRefTree idt = RandomDirectoryTree.randomDirTree(TSIZE,DSIZE,DPROB,VAR,DVAR, 
        new Random(RND_SEED),kg);
    ...
    oprnd = new Random(RND_SEED);
    kg = new KeyGen( RandomDirectoryTree.KEY_GEN );
    idt = RandomDirectoryTree.randomDirTree(TSIZE, DSIZE, DPROB, VAR, DVAR, new Random(RND_SEED),kg);
    ((RandomDirectoryTree.MutableDirectoryTree ) idt).setOrdered(true);
