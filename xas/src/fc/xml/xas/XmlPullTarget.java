...
public class XmlPullTarget implements SerializerTarget {
    ...
    public XmlPullTarget (XmlSerializer ser, String type, OutputStream out,
	...
	case Item.START_TAG: {
		...
		if (value instanceof ParsedPrimitive) {
			...
			if (vqn.getNamespace().equals(XasUtil.XSD_NS)) {
			    stringValue = "xsd:" + vqn.getName();
			} else if (vqn.getNamespace().equals(
			    "http://www.hiit.fi/fuego/fc/demos/captio")) {
			    stringValue = "demo:" + vqn.getName();
			} else if (vqn.getNamespace().equals(
			    "http://www.hiit.fi/fuego/fc/test")) {
			    stringValue = "test:" + vqn.getName();
			} else if (vqn
			    .getNamespace()
			    .equals(
				"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd")) {
			    stringValue = "wss:" + vqn.getName();
			}
		    }
	...
	case Item.PI: {
	    Pi p = (Pi) item;
	    StringBuffer sb = new StringBuffer(p.getTarget());
