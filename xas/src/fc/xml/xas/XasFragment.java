 ...
 */
public class XasFragment extends FragmentItem implements Queryable {

    public static final int LIST_FRAGMENT = 0x4200;
    private boolean detached = false;

    public XasFragment (Collection items, Item firstItem) {
	this(LIST_FRAGMENT, items, firstItem);
    }

    protected XasFragment (int type, Collection items, Item firstItem) {
	super(type, 1);
	...
	if (!Item.isStartTag(item)) {
	    throw new IllegalArgumentException(String.valueOf(item) + " at "
		    + offset + " not a start tag");
	}
    }

    /*
         * private void checkEnd (int offset) { Item item = get(offset); if (!
         * (item instanceof EndTag)) { throw new
         * IllegalArgumentException(String.valueOf(item) + " at " + offset + "
         * not an end tag"); } }
         */
	...
	if (Item.isStartTag(item)) {
	    ...
	    if (Item.isEndTag(end)
		    && ((EndTag) end).getName().equals(
			((StartTag) item).getName())) {
	...
	for (int i = 0; i < path.length; i++) {
	    ...
	    item = pointer.get();
	    if (Item.isEndTag(item) || Item.isEndDocument(item)) {
		return null;
	    }
	}
	...
	for (int i = 0; i < n; i++) {
		...
		Integer si = (Integer) stack.remove(stack.size() - 1);
		setDirect(si.intValue(), new XasFragment(this, si.intValue(), i
			- si.intValue() + 1));
	    }
	}
    }
