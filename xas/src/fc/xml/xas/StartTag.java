...
public class StartTag extends Item {
    ...
    private void insertAttribute (AttributeNode curr, AttributeNode prev,
	    AttributeNode next) {
    ...
    private void removeAttribute (AttributeNode curr, AttributeNode prev,
	    Qname name) {
	if (curr != null) {
	...
	if (prefixes != null) {
	    ...
	    throw new IllegalStateException("No namespace found for prefix "
		+ prefix + " in mapping " + prefixes);
	}
	return namespace;
    }
	...
	if (prefixes != null) {
	    ...
	    throw new IllegalStateException("No prefix found for namespace "
		+ namespace + " in mapping " + prefixes);
	}
	return prefix;
    }
	...
	while (pn != null) {
	    if (pn.getNamespace().equals(namespace)) {
		return pn.getPrefix();
	    }
	    pn = pn.getNext();
	}
	...
	if (parent != null) {
		...
		if (initPrefixes != null) {
		    return new DetachedPrefixIterator(new MergePrefixIterator(
			localPrefixes(), parent.sortedDetachedPrefixes()));
		} else {
		    return allPrefixes();
		}
	    } else {
	...
	if (this == o) {
		...
		for (a1 = atts, a2 = st.atts; a1 != null && a2 != null; a1 =
		    a1.getNext(), a2 = a2.getNext()) {
		    if (!a1.equals(a2)) {
			return false;
		    }
		}
    ...
    /** Create a copy that has another context. The copy has
     * the same Qname and attributes as this object, but another context.
     * That is, the prefix mappings of this object are not carried over
     * to the copy. 
     * <p>Note: The attribute list is not shared between the objects.
     * 
     * @param ctx new context of the tag
     */
	...
	public MergePrefixIterator (Iterator<PrefixNode> left,
		Iterator<PrefixNode> right) {
	    this.left = left;
	    this.right = right;
	    updateLeft();
	    updateRight();
	...
	public void remove () {
		...
		"Remove not supported for prefixes");
	}
	...
	public PrefixNode next () {
		...
		throw new NoSuchElementException("No more prefixes, current="
