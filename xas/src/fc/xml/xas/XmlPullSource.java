...
public class XmlPullSource implements ParserSource {
    ...
    public XmlPullSource (XmlPullParser parser, InputStream in)
	    throws IOException {
	this.parser = parser;
	try {
	    parser.setFeature(XmlPullParser.FEATURE_PROCESS_NAMESPACES, true);
	...
	if (inProgress) {
		...
		try {
			...
			StartTag st = new StartTag(new Qname(parser
			    .getNamespace(), parser.getName()), (StartTag) sts.peek());
			int start = parser
			...
			for (int i = 0; i < atts; i++) {
			    st.addAttribute(new Qname(parser
				.getAttributeNamespace(i), parser
				.getAttributeName(i)), parser
				.getAttributeValue(i));
			}
			sts.push(st);
			result = st;
			...
			result = new EndTag(new Qname(parser.getNamespace(),
			    parser.getName()));
			sts.pop();
			break;
			...
			if (index >= 0) {
			    ...
			    if (index >= 0) {
				String name = text.substring(0, index).trim();
				String systemId = text.substring(index + 6)
				    .trim();
    ...
    private void doSetContext(StartTag context) {
	if( context != null ) {
	    doSetContext(context.getContext());
	}
	sts.push(context);	    
    }
