...
/**
 * A collection of general purpose verification methods. This class provides a
 * collection of methods useful in verifying method arguments. In addition to
 * very generic methods, several methods are provided to verify XML-related
 * constraints.
 */
public class Verifier {
	...
	if (!condition) {
	    throw new IllegalStateException("Verification of condition <"
		    + message + "> failed");
	}
    }
	...
	if (o != null) {
	    throw new IllegalStateException("Attempting to write a write-once"
		    + " value");
	}
    }
	...
	if (o1.equals(o2)) {
	    throw new IllegalArgumentException(String.valueOf(o1) + " == " + o2);
	}
    }

    public static void checkPositive (int n) {
	if (n <= 0) {
	    throw new IllegalArgumentException(String.valueOf(n) + " <= 0");
	}
    }
	...
	if (x > y) {
	    throw new IllegalArgumentException(String.valueOf(x) + " > " + y);
	}
    }
	...
	if (!c.isInstance(o)) {
	    throw new IllegalArgumentException("Object " + o + " of class "
		    + o.getClass() + " not an " + "instance of " + c);
	}
    }
    ...
    public static void checkOffsetLength (int min, int offset, int length,
	    int max) {
    ...
    public static void checkFragment (Collection items) {
	checkNotNull(items);
    ...
    public static StartTag verifyStartTag (Item item, Qname name)
	    throws IOException {
	if (!Item.isStartTag(item)) {
	    throw new IOException("Expected start tag, got " + item);
	}
	StartTag st = (StartTag) item;
	if (name != null && !st.getName().equals(name)) {
	    throw new IOException("Expected start tag with name " + name
		    + ", got " + st.getName());
	}
	return st;
    }

    public static EndTag verifyEndTag (Item item, Qname name)
	    throws IOException {
	if (!Item.isEndTag(item)) {
	    throw new IOException("Expected end tag, got " + item);
	}
	EndTag et = (EndTag) item;
	if (name != null && !et.getName().equals(name)) {
	    throw new IOException("Expected end tag with name " + name
		    + ", got " + et.getName());
	}
	return et;
    }
