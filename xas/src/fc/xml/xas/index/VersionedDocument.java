...
/**
 * A document that supports mutations while preserving pointers to itself. Any
 * pointers acquired from such a document with {@link #pointer()} or
 * {@link #query(int[])} implement {@link fc.xml.xas.MutablePointer} so they can
 * be used to mutate the document. Also, any such pointers will always follow
 * their pointed-to item throughout these changes (note that a delete may
 * invalidate such a pointer).
 */
public class VersionedDocument extends Document {
    ...
    protected Pointer constructPointer (DeweyKey key,
    ...
    void moveAfter (DeweyKey source, FragmentPointer sourcePointer,
	    DeweyKey target, FragmentPointer targetPointer) {
	version = version.moveAfter(source, sourcePointer, target,
	    targetPointer);
    }

    void moveTo (DeweyKey source, FragmentPointer sourcePointer,
	    DeweyKey target, FragmentPointer targetPointer) {
