...
/**
 * A fragment that can be created lazily from an external source. A lazy
 * fragment is <em>unforced</em> right after creation, i.e., it does not
 * actually contain any items. There are two different ways to <em>force</em>
 * a fragment, i.e., cause it to parse itself into a sequence of items from the
 * external source. A lazy fragment can be forced <em>partially</em>, in
 * which case some of the items it contains are unforced lazy fragments, or
 * <em>completely</em>, in which case any contained lazy fragments are also
 * completely forced.
 */
public class LazyFragment extends FragmentItem implements Queryable {
    ...
    /**
         ...
         * 
         * @param index the index storing the entries corresponding to this
         *        fragment's document
         * @param key the key corresponding to this fragment, absolute from the
         *        document's root
         * @param firstItem the first item in the created fragment
         */
    ...
    private static void copyStream (InputStream in, String inEncoding,
	    int length, OutputStream out, String outEncoding)
	    throws IOException {
	if (Log.isEnabled(Log.TRACE)) {
	    Log.log("copyStream(" + in + ", " + inEncoding + ", " + length
		+ ", " + out + ", " + outEncoding + ")", Log.TRACE);
	}
	if (inEncoding.equalsIgnoreCase(outEncoding)) {
	    IOUtil.copyStream(in, out, length);
	    out.flush();
	} else {
	    throw new UnsupportedOperationException("Input charset="
		+ inEncoding + ", output charset=" + outEncoding
	...
	if (isEvaluated()) {
		...
		copyStream(ps.getInputStream(), ps.getEncoding(), entry
		    .getLength(), st.getOutputStream(), st.getEncoding());
    ...
    /**
         ...
         * 
         * @throws IOException if parsing the external source during forcing
         *         fails
         */
    ...
    /**
         * For this fragment partially up to a given depth. Any contents below
         * the given depth will be left unforced.
         * 
         * @param depth the maximum depth to be forced, <code>0</code> is the
         *        root of this fragment (and hence <code>force(0)</code> has
         *        no effect)
         * 
         * @throws IOException if parsing the external source during forcing
         *         fails
	...
	while (source.getCurrentPosition() < end) {
	    if (Log.isEnabled(Log.TRACE)) {
		Log.log("force, pos=" + source.getCurrentPosition() + ", end="
		    + end, Log.TRACE);
	    }
	    if (current < depth) {
		...
		if (Item.isStartTag(item)) {
		    ...
		    k = k.up();
		    if (!k.isRoot()) {
			throw new IOException("Reached EndDocument at key " + k);
		    }
		} else {
		    k = k.next();
		    isText = false;
		...
		passUntil(k, stack.empty() ? null : (StartTag) stack.peek(),
		    end);
		source.setPosition(source.getCurrentPosition(), entry
		    .getContext());
		current -= 1;
		k = k.up();
	    }
    ...
    private void passLevel (DeweyKey k, StartTag context, int end)
	    throws IOException {
	if (Log.isEnabled(Log.TRACE)) {
    ...
    private void passUntil (DeweyKey k, StartTag context, int end)
	    throws IOException {
	if (Log.isEnabled(Log.TRACE)) {
	    Log.log("passUntil(" + k + ", " + context + ", " + end + ")",
		Log.TRACE);
	}
	SeekableSource source = index.getSource();
	boolean isText = false;
    ...
    private FragmentItem forceLevel (List l, int i) throws IOException {
	if (Log.isEnabled(Log.TRACE)) {
	    Log.log("forceLevel(" + l + ", " + i + ")", Log.TRACE);
	}
	if (i == 0) {
	    DeweyKey k = (DeweyKey) l.get(0);
	    ...
	    DeweyKey k = (DeweyKey) l.get(i);
	    DeweyKey c = (DeweyKey) l.get(i - 1);
	    Index.Entry e = index.find(k);
    ...
    /**
         * Force this fragment partially up to a certain key. This method
         * ensures that the fragment identified by the given key will be forced.
         * Otherwise the forcing is minimal, i.e., any indexed subtree of the
         * document whose key is not a prefix of the given key will be left
         * unforced.
         * 
         * @param k the key of the node to guarantee is forced
         * @return the forced fragment, if this fragment was unforced
         *         previously, <code>null</code> otherwise
         * 
         * @throws IOException if parsing the external source during forcing
         *         fails
         */
	...
	if (isEvaluated()) {
	    ...
	    throw new IllegalArgumentException("Argument key " + k
		+ " not in subtree of key " + key);
	}
    }
	...
	if (Log.isEnabled(Log.TRACE)) {
	    Log.log("query(" + Util.toString(path) + "),this=" + this,
		Log.TRACE);
	}
	int offset = key.size();
	if (!isEvaluated()) {
	...
	for (int i = offset; i < path.length; i++) {
		...
		if (!lf.isEvaluated()) {
		    try {
			if (lf.force(DeweyKey.construct(path)) == null) {
			    return null;
			}
		    } catch (IOException ex) {
			Log.warning("Query failed", ex);
