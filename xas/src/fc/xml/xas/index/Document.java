...
/**
 * A class representing a complete document. Essentially this class is just a
 * wrapper around a fragment, and its benefit comes from the
 * {@link GlobalPointer} that is used to access and mutate it.
 */
public class Document implements Queryable {
	...
	if (pointer != null) {
	    return constructPointer(DeweyKey.construct(path),
		(MutableFragmentPointer) pointer);
	} else {
	    return null;
	}
