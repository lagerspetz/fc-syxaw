...
public class PrefixNode implements Comparable<PrefixNode> {
	...
	for (PrefixNode n = this; n != null; n = n.getNext()) {
		...
		break;
	    } else if (!emptyMapped && n.getPrefix().length() == 0
		&& n.getNamespace().length() > 0) {
		emptyMapped = true;
	    }
	}
	if (namespace == null) {
		...
		throw new IllegalStateException(
		    "No namespace found for prefix " + prefix + " in mapping "
			+ this);
	    }
	}
	return namespace;
	...
	if (prefix == null) {
		...
		throw new IllegalStateException(
		    "No prefix found for namespace " + namespace
			+ " in mapping " + this);
	    }
	}
	return prefix;
	...
	StringBuilder result =
	    new StringBuilder("{" + prefix + "=" + namespace + "}");
	if (next != null) {
	    result.append(" ");
