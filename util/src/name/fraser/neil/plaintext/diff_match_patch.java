 ...
 * <p>NOTE(fc-fp): Some functions have been stripped out to make the class work on fp.
...
public class diff_match_patch {
    ...
    for (int d = v_map.size() - 2; d >= 0; d--) {
      while (true) {
        if (v_map.get(d).contains((x - 1) + "," + y)) {
          ...
          x--;
          y--;
          //A! assert (text1.charAt(x) == text2.charAt(y))
          //A!       : "No diagonal.  Can't happen. (diff_path1)";
    ...
    for (int d = v_map.size() - 2; d >= 0; d--) {
      while (true) {
        if (v_map.get(d).contains((x - 1) + "," + y)) {
          ...
          x--;
          y--;
          //A! assert (text1.charAt(text1.length() - x - 1)
          //A!        == text2.charAt(text2.length() - y - 1))
          //A!       : "No diagonal.  Can't happen. (diff_path2)";
          if (last_op == Operation.EQUAL) {
            path.getLast().text += text1.charAt(text1.length() - x - 1);
          } else {
          ...
          if (count_delete != 0 && count_insert != 0) {
            ...
            if (commonlength != 0) {
              if (pointer.hasPrevious()) {
                thisDiff = pointer.previous();
                //A! assert thisDiff.operation == Operation.EQUAL
                //A!       : "Previous diff should have been an equality.";
    ...
    for (Diff aDiff : diffs) {
      ...
      case INSERT:
        html.append("<INS STYLE=\"background:#E6FFE6;\" TITLE=\"i="
            + aDiff.index + "\">" + text + "</INS>");
        break;
      case DELETE:
        html.append("<DEL STYLE=\"background:#FFE6E6;\" TITLE=\"i="
            + aDiff.index + "\">" + text + "</DEL>");
        break;
      case EQUAL:
        html.append("<SPAN TITLE=\"i=" + aDiff.index + "\">" + text
            + "</SPAN>");
        break;
      }
    }
  ...
  public String diff_toDelta(LinkedList<Diff> diffs) {
    ...
    for (Diff aDiff : diffs) {
      ...
      case INSERT:
        try {
          txt.append("+" + URLEncoder.encode(aDiff.text, "UTF-8")
                                     .replace('+', ' ') + "\t");
  ...
  public int match_bitap(String text, String pattern, int loc) {
    //A! assert (Match_MaxBits == 0 || pattern.length() <= Match_MaxBits)
    //A!    : "Pattern too long for this application.";
   ...
   * @return List of Patch objects or null if invalid
  ...
  */
  ...
  public static class Patch {
    ...
    public String toString() {
      ...
      // In speed critical applications this could be dropped since the
      // receiving application will probably decode these fine.
      String txtString = txt.toString().replace("%21", "!").replace("%7E", "~")
          .replace("%27", "'").replace("%28", "(").replace("%29", ")")
          .replace("%3B", ";").replace("%2F", "/").replace("%3F", "?")
          .replace("%3A", ":").replace("%40", "@").replace("%26", "&")
          .replace("%3D", "=").replace("%2B", "+").replace("%24", "$")
...
// arch-tag: d5e0f256-f5fc-4cd8-ae5d-644b79d3cf1b
//
