...
public class RefTrees {
   ...
   * is sufficient to supply a set whose {@link java.util.Set#contains contains}
  ...
  protected static RefTreeNodeImpl expandRefs( RefTreeNode root, Set allowedRefs,
    ...
    if ( allowedRefs.contains( root.getId())) {
      // Put a tree ref
      newRoot = new RefTreeNodeImpl(parent,root.getId(),
          new TreeReference(root.getId()));
    } else if ( ( root.isReference() || inBackingTree ) &&  
                allowedContent.contains(root.getId()) ) {
      // BUGFIX-20061212-1: (Related) prettified if-clause boolean expression
      // Put a node ref if the node is allowed and not an expanded node in 
      // the source tree, i.e. the root is a ref, or we are traversing in the
      // backing tree (which happened because the source tree did a tree-ref
      // higher up)
      newRoot = new RefTreeNodeImpl(parent,root.getId(),
          new NodeReference(root.getId()));
    ...
    if( !newRoot.isTreeRef() ) {
      ...
      // This happened because the jump into the backingTree wasn't "sticky", 
      // i.e., the backingTree flag could turn false although we had already
      ...
      for (Iterator i =
           inBackingTree ? backingTree.getNode(root.getId()).getChildIterator() :
   ...
   * a common backing tree (and only that tree), is <i>normalized</i>, iff
   * there are no tree references for which a descendant of the reference target
   ...
   * presence or absence of an entire subtree .
   *
...
a*  d'
   ...
   * with {@link #expandRefs(RefTreeNode, Set, Set, IdAddressableRefTree, RefTreeNode, boolean) 
   * expandRefs}.
   *
   * @param backingTree tree containing all reference targets
  ...
  public static Set[] normalize( IdAddressableRefTree backingTree,
    ...
    for (int iTree = 0; iTree < trees.length; iTree++) {
      ...
      addTreeRefs(backingTree, trees[iTree].getRoot(),
                  commonRefs, innerNodes, trees[iTree], expC,
                  matching);
      if( expC != null )
        expandedContents[iTree] = expC;
    }
    //return commonRefs;
  ...
  protected static void addTreeRefs(IdAddressableRefTree backingTree,
                                    ...
                                    RefTree treeTag, Set expandedContents,
                                    KeyMap matching ) throws
      NodeNotFoundException {
    //A! assert root != null && backingTree!=null;
    if ( root.isReference() 
        || backingTree.contains( getBackTarget( root, matching ) ) )
    ...
    if( expandedContents != null && !root.isReference() )
      expandedContents.add(root.getId()); // FIXME: What key space to use?
      // We now just put front keys, but these may be incoherent across trees
      // and should be mapped somehow. OK-- Let's do it like this at some point:
    ...
    for ( Iterator i = root.getChildIterator(); i.hasNext();)
      addTreeRefs(backingTree, (RefTreeNode) i.next(), commonRefs, innerNodes,
                  treeTag, expandedContents, matching);
  }

  protected static void downscan(RefTreeNode bnode,
  ...
  protected static void prohibit(Key bid, Map treeRefs, Set inner, 
      IdAddressableRefTree bt )
      throws NodeNotFoundException {
    if (bid == null || inner.contains(bid) || bt.getNode(bid) == null)
    ...
    for (Iterator /*!5 <Key> */ i = bt.childIterator(bid); i.hasNext(); ) {
      Key cid = (Key) i.next();
  ...
  protected static void addRefToRefset(Key bid, RefTree treeTag, Map treeRefs,
                                       Set inner,IdAddressableRefTree bt) throws
      NodeNotFoundException{
    prohibit(bt.getParent(bid), treeRefs, inner, bt);
    if (treeRefs.containsKey(bid))
      ( (Set) treeRefs.get(bid)).add(treeTag);
    else {
  ...
  // refNodes move to innerNodesB, and the allowed descendants of a ref replace the
  // ref in refLeavesN
  ...
  // downscan()
  //
  // NOTE: Since refLeavesB and innerNodesB always deals with subtrees of backingTree,
  ...
  protected static void addTreeRef(IdAddressableRefTree backingTree,
                                   ...
                                   Set refInnerNodes, RefTree treeTag,
                                   KeyMap matching ) throws
      NodeNotFoundException {
...
//    Log.log("Enter addRef, id="+id+" leaves="+refLeaves+", inner="+refInnerNodes,Log.INFO);
    ...
    if( node.isTreeRef() )
      addRefToRefset(backId,treeTag,refLeaves,refInnerNodes,backingTree);
    else
      prohibit(backId,refLeaves,refInnerNodes,backingTree);
    for (RefTreeNode scan = node.getParent(); scan != null;
         scan = scan.getParent()) {
      prohibit( getBackTarget( scan, matching ),refLeaves,refInnerNodes,
          backingTree);
    }
  }

  protected static Key getBackTarget( RefTreeNode n, KeyMap m ) {
    return n.isReference() ? n.getReference().getTarget() : 
        m.getBackKey(n.getId());
  }

  protected static RefTreeNode getBackMatch( RefTreeNode n, KeyMap m, 
      IdAddressableRefTree bt ) {
  ...
  */
   ...
   * tree to a target tree means that the target tree is re-shaped to be
   * isomorphic to the source tree. The re-shaping takes place through the
   ...
   * a vast amount of nodes. Also note that parts of the target tree that are
   * referenced by tree references from the source tree need not be traversed.
   *
   * @param source source tree
   * @param target target tree to restructure
   * @param delRoots roots of subtrees to delete from target in order to
  ...
  public static void apply(RefTree source, MutableRefTree target, Set delRoots,
    ...
    if( delRoots == null ) {
      // Autoclean
      for (Iterator /*!5 <Key> */ i = deletia.iterator(); i.hasNext(); )
        target.delete( (Key) i.next());
    }
  }
  ...
  protected static void apply(RefTree newTree, MutableRefTree target,
    ...
    Key currentLocationT = currentNewNode.isReference() ?
        currentNewNode.getReference().getTarget() : 
    ...
    // and content ops). This will lead to an anomaly when renaming and moving
...
//        Log.log("Skipping identical child list...",Log.INFO);
    ...
    // Check for deleted, moved away and updated nodes, and add these to delRoots
    Set visitedChildren = new HashSet(); // Contains new/src ids
    // IMPORTANT since dest id's may be allocated on the fly
    RefTreeNode currentT = target.getNode(currentLocationT); //  Never null, since current loc always in both trees
    ...
    for (Iterator i = currentNewNode.getChildIterator(); i.hasNext(); ) {
      ...
      if (oldLocation == null) {
        if( newNode.isReference() )
          // Trying to insert a reference; not possible since a reference MUST
...
///      resurrect(newIdT);
  ...
  /*
  protected static void apply2(RefTree newTree, MutableRefTree target,
      RefTreeNode currentNewNode, RefTreeNode currentTargetNode, Set<Key> delRoots,
      KeyMap newToTarget, KeyMap targetToOrigTarget ) throws
      NodeNotFoundException {
    
    Key currentKeyN = currentNewNode.getId();
    //Key currentLocationT = newToThis.getDestId(currentNewNode);
...
//    Log.log("Reftree @ "+currentLocation+", stopping recursion.",Log.INFO);
    ...
    if (!currentNewNode.isReference())
      target.update(currentTargetNode.getId(), currentNewNode.getContent());

    
    // Make set of new child list keys on oldTargetSpace 
    Set<Key> newNodeChildMap = new HashSet();
    {
      Iterator i = currentNewNode.getChildIterator();
      if( i == IDENTICAL_CHILDLIST ) {
        return; // Child list matches target tree
      ...
      for (;i.hasNext(); ) {
        RefTreeNode child = (RefTreeNode) i.next();
        Key otk = child.isReference() ?
            child.getReference().getTarget() :
            targetToOrigTarget.getBackKey(
                newToTarget.getBackKey( child.getId()) );
        if( otk != null )    
          newNodeChildMap.add( otk );
      }
    }
    // Check for deleted, moved away and updated nodes, and add these to delRoots
    
    Set<Key> oldTargetKeysInBoth = new HashSet<Key>(); // Contains new/src ids
    //  IMPORTANT since dest id's may be allocated on the fly
    for (Iterator i = currentTargetNode.getChildIterator(); i.hasNext(); ) {
      RefTreeNode cchild = (RefTreeNode) i.next();
      Key tk=cchild.getId();
      if (!newNodeChildMap.contains(tk)) {
        delRoots.add(tk); // Tentative delete. May also have been moved somewhere...
      } else
        oldTargetKeysInBoth.add(tk);
    }
    ...
    for (Iterator i = currentNewNode.getChildIterator(); i.hasNext(); ) {
      RefTreeNode newNode = (RefTreeNode) i.next();
      Key otk = newNode.isReference() ?
          newNode.getReference().getTarget() :
          targetToOrigTarget.getBackKey(
              newToTarget.getBackKey( newNode.getId()) );
            
      if (oldTargetKeysInBoth.contains(otk)) {
        ; // Already processed
      } else
      // Now, we have a new and a target node, that is either missing
      // or not in the current target childlist
      ...
      if ( otk == null ) {
        ...
        //  exist in the target tree
          throw new NodeNotFoundException(newNode.getId());
        target.insert(currentTarget, 
            newToTarget.getBackKey(newNode.getId()),
            newNode.getContent());
      } else {
        // the target key is in the target tree, but not in the childlist ->
        // move get it here
        if (delRoots.contains(otk)) {
          //Log.log("Found tentative delete: " + newIdT, Log.INFO);
          delRoots.remove(otk);
        }
        target.move( targetToOrigTarget.getFrontKey(otk),
...
//  away)
   ...
   * <p><b>NOTE:</b> This class is being deprecated in favor of  
   * {@link KeyMap}. Use that instead.
   */
  ...
  public static abstract class IdMap {
     ...
     */
    public abstract Key getDestId(Key srcId, RefTreeNode src) throws
        NodeNotFoundException;
    ...
    public abstract Key getSrcId(Key dstId, RefTreeNode dest) throws
        NodeNotFoundException;
    ...
    public final Key getDestId(Key srcId) throws
        NodeNotFoundException {
    ...
    public final Key getSrcId(Key dstId) throws
        NodeNotFoundException {
    ...
    public final Key getDestId(RefTreeNode src) throws
        NodeNotFoundException {
     ...
     * @return id in the source tree.
     */
    public final Key getSrcId(RefTreeNode dest) throws
        NodeNotFoundException {
  ...
  public static IdAddressableRefTree getRefTree( final Key rootId ) {
    return getAddressableTree(new RefTreeImpl(
        new RefTreeNodeImpl(null,rootId, new TreeReference(rootId))));
  }

  /** Get a single-node tree reference reftree. Return a reftree
   * consisting of a single node with content given by
   * <code>n</code>. The key of the root is not defined, and may not
   * be serializable. 
   *
   * @param o content of sole node in tree
   * @return a single node reference tree
   */

  public static RefTree getRefTree( Object o ) {
    return new RefTreeImpl(new RefTreeNodeImpl(null,
        TransientKey.createKey(),o));
  }
  ...
  public static IdAddressableRefTree getAddressableTree( RefTree t) {
    ...
    return t instanceof IdAddressableRefTree ?  // Don't build unnecessary indexes 
  ...
  /** Test is a tree is a tree reference to a specified target. The method
   * tests if the root node of t is a tree reference to target. Useful for
   * testing tree equality by tree reference.
   * 
   * @param t tree to test
   * @param target target key
   * @return <code>true</code> if <i>t</i> is a tree reference 
   * to <i>target</i>
   */
  
  public static boolean isTreeRef(RefTree t, Key target ) {
    if( t.getRoot().isTreeRef() &&  
        t.getRoot().getReference().getTarget().equals(
            target) ) 
      return true;
    return false;
  }
   ...
   * is unnecessarily expensive to expand the child list of a node as a list of
  ...
  // Dfs traversal of reftree
  public static Iterator<RefTreeNode> dfsIterator(final RefTree t) {
    return dfsIterator(t.getRoot());
  }
  
  public static Iterator<RefTreeNode> dfsIterator(final RefTreeNode node) {
    final Stack iterators = new Stack();
    return new Iterator<RefTreeNode>() {

      RefTreeNode next = null;
      Iterator i = node.getChildIterator();
      
      public RefTreeNode setNext() {
        if( next != null )
          return next;
        if( i == null )
        ...
        if( !i.hasNext() ) {
          ...
          i = (Iterator) iterators.pop();
          return setNext();
        }
        next = (RefTreeNode) i.next();
        Iterator i2 = next.getChildIterator();
        if( i2.hasNext() )  {
          iterators.push(i);
          i = i2;
        }
  ...
  public static Iterator<Key> dfsKeyIterator(final RefTree t) {
    return dfsKeyIterator(t.getRoot());
  }

  public static Iterator<Key> dfsKeyIterator(final RefTreeNode node) {
    final Stack iterators = new Stack();
    return new Iterator<Key>() {
      Iterator<RefTreeNode> i = dfsIterator(node);

      public Key next() {
        RefTreeNode next = i.next();
