...
/*      Key id = n.getId();
  ...
  protected static Key ensureSuccessor(Key baseId, Map /*!5 <Key,Key> */ succcessors,
                                     IdAddressableRefTree base) {
    final Key NO_PREDECESSOR = StringKey.createUniqueKey();
    Key succ = (Key) succcessors.get(baseId);
    try {
      if (succ == null) {
        Key pid = base.getParent(baseId);
        Key prev = NO_PREDECESSOR;
        for (Iterator /*!5 <Key> */ i = base.childIterator(pid); i.hasNext(); ) {
          Key nid = (Key) i.next();
        ...
        succ =  (Key) succcessors.get(baseId);
        if( succ == null )
  ...
  protected static class CopySequenceIterator implements Iterator {
    ...
    protected Map /*!5 <Key,Key> */ successors = new HashMap(); // = new HashMap(); // Move out of iterator for more speed!!!
    ...
    public Object decode(PeekableItemSource is, KeyIdentificationModel kim ) 
      ...
      if( i.getType() == Item.START_TAG ) {
        ...
        if( DIFF_COPY_TAG.equals( st.getName()) )  {
          ...
          Long run=null;
          for( Iterator /*!5 <AttributeNode> */ it = st.attributes(); it.hasNext(); ) {
            ...
            if (DIFF_CPYSRC_ATTR.equals(an.getName()))
              ...
              try {
                if( an.getValue() instanceof Number )
                  run = new Long( ((Number) an.getValue()).longValue() );
                else
