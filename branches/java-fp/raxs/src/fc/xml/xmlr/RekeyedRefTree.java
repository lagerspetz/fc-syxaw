...
/** reftree with remapped keys. The object is a reftree with the same
 * structure and content as an existing reftree, but with the node keys
 * remapped according to a given keymap. 
 */ 
public class RekeyedRefTree extends AbstractMutableRefTree {
  ...
  private RekeyedRefTree(RefTree frontTree, KeyMap map) {
    this.map = map;
    ft = frontTree;
    ift = frontTree instanceof IdAddressableRefTree ? 
        (IdAddressableRefTree) ft : null;
    mft = frontTree instanceof MutableRefTree ?
        (MutableRefTree) ft : null;
  }
  
  /** Create remapped tree. The remapped tree is considered the front tree,
   * and the original tree the back tree.
   * 
   * @param frontTree tree with keys to remap.
   * @param map Key map
  ...
  /** @inheritDoc */
  /*!5 @Override */
  public Key insert(Key parentId, long pos, Key newId, Object content) throws NodeNotFoundException {
    //A! assert !(content instanceof Reference) : "ref insert not coded"; // FIXME
  ...
  private class RekeyNode implements RefTreeNode {
    ...
    public Key getId() {
      if( cachedKey == null ) {
        Key bk =  n.isReference() ? n.getReference().getTarget() :
            map.getBackKey(n.getId());
  ...
  private static class LocalKey implements Key {
    ...
    public boolean equals(Object o) {
      return o instanceof LocalKey &&
        o != null &&
        Util.equals( scope, ((LocalKey) o).scope) &&
  ...
  /** Tree that re-maps reference targets by a keymap. Use with algorithms
   * that do not have a keymap parameter. */
    ...
    private class ReRefNode implements RefTreeNode  {
      ...
      public Reference getReference() {
        ...
        if( r.isTreeReference() ) {
          ...
          return new NodeReference( map.getFrontKey( r.getTarget() ) );          
      }
