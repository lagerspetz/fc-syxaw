...
/** XMLR-Compatible Dewey key. Works as {@link fc.xml.xas.index.DeweyKey},
 * but implements the {@link fc.xml.xmlr.Key} interface.
 * <p><b>NOTE</b>: This class will probably be merged with  
 * {@link fc.xml.xas.index.DeweyKey} at some point.
 * 
 */
public class DeweyKey implements Key {

  private final int[] EMPTY_PATH = new int[] {};
  public static final DeweyKey ROOT_KEY =
    new DeweyKey(fc.xml.xas.index.DeweyKey.root());
  ...
  public static DeweyKey createKey(String s) throws IOException {
    ...
    for( int i = 0;i<s.length();i++) {
      ...
      if( Character.isDigit(ch) ) {
        ...
        throw new IOException("Bad Dewey key "+s);
    }
    ...
    for( int i = 0;i<s.length();i++) {
      ...
      if( Character.isDigit(ch) ) {
        ...
        //A! assert '0'<=ch && ch <= '9' : "Cannot handle digit "+ch; 
        nums+=lastWasDelim;
        lastWasDelim = 0;
        ...
        ; //A! assert false; // Should never get here
    }
  ...
  public DeweyKey up() {
    if( k.isRoot() )
      return null; //throw new IllegalArgumentException("root key has no .up()");
  ...
  /** Default key and identification model for Dewey keys. The key model parses
   * keys from strings to Dewey keys. The identification model simply returns
   * MutableRefTree.AUTO_KEY for any item, and dow not tag output items in any 
   * way.
   */
  
  public static KeyIdentificationModel KEY_IDENTIFICATION_MODEL = 
    new KeyIdentificationModel(  
      new KeyModel() {
