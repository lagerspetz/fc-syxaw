...
/** Codec for translating between reftree node content and XAS items. A
 * XasCodec is not normally responsible for translating reference nodes;
 * for that see {@link XasCodec.ReferenceCodec}.
 *
 */
public interface XasCodec {
   ...
   * 
   * @param is items to decode
   * @param kim used key identification model
   * @return node content
   * @throws IOException if an I/O error occurs
   */
  public Object decode(PeekableItemSource is, KeyIdentificationModel kim ) throws IOException;

  /** Encode content of node as XAS items. <b>Note:</b> If yo do not use 
   * the passed XAS context to when creating a start tag, that tag, and any of 
   * its descendants, won't have access to any prefix mappings defined previously
   * in the document. (This is a feature of the XAS API).   
   * 
   * @param t item target
   * @param n node to encode
   * @param context Current context of the item target
   * @throws IOException if an I/O error occurs
   */
  ...
  // decode/encode
  /** Tagging interface that indicates that the codec handles decoding and
   * encoding of reference nodes.
   */
  ...
  public abstract static class EncoderOnly implements XasCodec {
    ...
    public final Object decode(PeekableItemSource is, KeyIdentificationModel kim) 
      throws IOException {
      //A! assert false: "This is not a decoder";
      return null;
    }
  }
  ...
  public abstract static class DecoderOnly implements XasCodec {
    ...
    public void encode(ItemTarget t, RefTreeNode n, StartTag context) throws IOException {
      //A! assert false: "This is not a encoder";      
    }
  }
  
  /** Default delegating codec for references. The codec handles reference
   * item encoding/decoding, and delegates the rest to the underlying codec.
   */
  
  public static class DefaultReferenceCodec 
    ...
    public Object decode(PeekableItemSource is, KeyIdentificationModel kim)
      ...
      if( RefItem.isRefItem(first) ) {
        ...
        content = ri.isTreeRef() ?  
          (Reference) new TreeReference( kim.makeKey( ri.getTarget() ) ) :
            (Reference) new NodeReference( kim.makeKey( ri.getTarget() ) );
      } else {
