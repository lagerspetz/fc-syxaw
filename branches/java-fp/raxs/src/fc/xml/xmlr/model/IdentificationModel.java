...
public interface IdentificationModel {
   ...
   * @param km Key model use to construct key
   * @return item key
   * @throws IOException if an I/O error occurs
   */
  public Key identify( Item i, KeyModel km ) throws IOException ;
  
  /** Tag item with serialized key. Typically, this method would add
   * an <code>id</code> attribute to {@link fc.xml.xas.StartTag StartTag} items.
   * 
   * @param i item to tag
   * @param k key to tag the item with
   * @param km Key model use to serialize key
   * @return tagged item
   */
   ...
   * 
   */
  public static final IdentificationModel ID_ATTRIBUTE = 
    ...
    public final Qname ID_ATTR  = new Qname("","id");
      ...
      public Key identify(Item i, KeyModel km) throws IOException {
        if( i.getType() == Item.START_TAG ) {
          ...
          for(Iterator /*!5 <AttributeNode> */ ia = ri.getExtraAttributes();ia.hasNext();) {
            AttributeNode an = (AttributeNode) ia.next();
            if( ID_ATTR.equals( an.getName() ) )
              return km.makeKey(an.getValue());
          }
          return km.makeKey( ri.getTarget() );
        } else
          return km.makeKey(null);
      }
...
//          ((StartTag) i).addAttribute(ID_ATTR, k.toString());
          ...
          RefItem ri = (RefItem) i;
//          if( XasUtil.findAttribute(ID_ATTR, ri.getExtraAttributes()) == null )
