...
public class RandomAccessXmlStore implements BatchMutable {
  ...
  public int getCurrentVersion() {
    if( !currentVersionInited ) {
      ...
      //A! assert !getDeltaFile(hdir, currentVersion).exists(): "Failed to get max ver";
    }
  ...
  public static class RemoveSafeXasItems implements ItemTransform {
    ...
    protected LinkedList queue = new LinkedList /*!5 <Item> */();
  ...
  public static Map /*!5 <String,TreeMap<Integer,byte[]>> */ _repo =
    new HashMap /*!5 <String,TreeMap<Integer,byte[]>> */();
  
  private  TreeMap /*!5 <Integer,byte[]>  */ _deltas = null;
  
  private int _initStore(String key) {
    _deltas = (TreeMap) _repo.get(key);
    if( _deltas==null) {
      _deltas = new TreeMap /*!5 <Integer,byte[]> */();
      ...
      if( _deltas.isEmpty() )
        return FIRST_VERSION;
      Log.info("Reusing store "+key+" at rev "+(1+
	      ((Integer) _deltas.lastKey()).intValue()));
      return ((Integer) _deltas.lastKey()).intValue()+1;
    }
  }
  
  protected InputStream _getDeltaInStream(int version) throws IOException {
    byte[] data = (byte[]) _deltas.get(new Integer(version));
    if( data == null )
  ...
  protected OutputStream _getDeltaOutStream(final int version) {
    return new FilterOutputStream(new ByteArrayOutputStream()) {
      ...
      public void close() throws IOException {
        ...
        _deltas.put( new Integer( version ), data);
  ...
  private void _dumpHistory() {
    String vh = "";
    int size = 0;
    for( Iterator i = _deltas.entrySet().iterator(); i.hasNext(); ) {
      Map.Entry e = (Map.Entry) i.next();
      int asize = ((byte[]) e.getValue()).length; 
      size+= asize;
      vh+=e.getKey().toString()+": "+asize+" bytes ("+
        size+" bytes)\n";
    }
