...
import fc.xml.xmlr.test.RandomDirectoryTree.SafeIdNodePicker;
...
public class RandomEdits {
  ...
  public static void run(PrintStream out, String[] args, long tests) {
    ...
    Class[] MARKER_CLASSES = new Class[] { Measurements.class,
        RandomEdits.class };
    Class /*!5 <? extends Store> */ storeType = IdStore.class;
    if (STORE != null) {
      try {
        storeType = (Class /*!5 <? extends Store> */) Class.forName(STORE);
      } catch (Exception e) {
    ...
    Object[] T_POINTS = { Measurements.H_RAXS_OPEN, H_START_SCRIPT, H_END_SCRIPT,
        Measurements.H_RAXS_COMMIT, Measurements.H_RAXS_COMMIT_APPLY, 
        Measurements.H_STORE_APPLY_WRITETREE,
        Measurements.H_STORE_APPLY_WRITETREE_END, 
        Measurements.H_STORE_INIT, Measurements.H_RAXS_CLOSE };

    Object[] M_POINTS = { M_INITIAL, Measurements.M_STORE_PREOPEN, 
        Measurements.M_STORE_OPEN, M_CURRENT,
        Measurements.M_STORE_PRECLOSE, Measurements.M_STORE_CLOSE };
    ...
    for (long tsize = TSIZE; tsize <= TMAX; tsize += TSTEP) {
      ...
      List /*!5 <List<Edit>> */ scripts = new LinkedList /*!5 <List<Edit>> */();
      ...
      for (int edits = MAX_EDITS; edits <= MAX_EDITS; edits += EDIT_STEP) {
        ...
        ArrayList /*!5 <Edit> */ script = new ArrayList /*!5 <Edit> */();
      ...
      try {
        ...
        // 2. Open: start t, m
        int lap = 0;
        //A! assert scripts.size() == 1;
        for (int run = 1; run <= runs; run++) {
          if (runs > 1)
            log.setRun(run);
          if (Measurements.MEMORY)
            Measure.set(M_INITIAL, Debug.usedMemory());
          for ( Iterator is = scripts.iterator();is.hasNext();) {
            List /*!5 <Edit> */ script = (List) is.next();
        ...
        if ((tests & VERS_GET) != 0) {
          ...
          for (Iterator i = h.listVersions().iterator();i.hasNext();) {
      ...
      if( incrementalTree ) {
        ...
        NodePicker np = isDeweyKeyed ? 
            (NodePicker) new RandomDirectoryTree.TransientNodePicker(cb, lrnd) : 
              (NodePicker) new RandomDirectoryTree.SafeIdNodePicker(cb, lrnd);
      ...
      if( repickEdit ) {
        ...
        for( Iterator is = scripts.iterator();is.hasNext();) {
          List /*!5 <Edit> */ script = (List) is.next();
          for(  Iterator ie = script.iterator();ie.hasNext(); ) {
            Edit e = (Edit) ie.next();
  ...
  private static MutableRefTree testRandomEdits(RandomAccessXmlStore raxs,
      List /*!5 <Edit> */ script, int off, long tests, int len) throws IOException, NodeNotFoundException {
  ...
  private static void testRandomRead(RandomAccessXmlStore raxs,
      List /*!5 <Edit> */ script, int len, long tests) throws IOException, NodeNotFoundException {
    IdAddressableRefTree t = raxs.getTree();
    Time.stamp(H_START_SCRIPT);
    for( Iterator i =  script.iterator(); i.hasNext();) {
      Edit e = (Edit) i.next();
  ...
  private static void traverseTree(RefTreeNode root, long tests ) {
    if( ( tests & CONTENT ) != 0 )
      readContent(root);
    for( Iterator /*!5 <RefTreeNode> */ in = root.getChildIterator();in.hasNext();)
      traverseTree((RefTreeNode) in.next(),tests);
  }
  ...
  public static class Edit implements Operation {
    private int op;
    private Key src;
    private Key target;
    private long pos;
    ...
    public Edit(int op, Key src, Key target, long pos, Object data) {
      this.op = op;
      this.src = src;
      this.target = target;
      this.pos = pos;
    ...
    public static void apply(List /*!5 <Edit> */ script, MutableRefTree t) throws NodeNotFoundException {
      for( Iterator i =  script.iterator(); i.hasNext();) {
        Edit e = (Edit) i.next();
        e.apply(t);
      }
    ...
    public static void apply(List /*!5 <Edit> */ script, MutableRefTree t, int start, int end) throws NodeNotFoundException {
      for( int i =start;i<end;i++) {
        //XmlrDebug.dumpTree(t);
        //Log.debug("Applying "+script.get(i));
        ((Edit) script.get(i)).apply(t);
      }
    }
  ...
  public static class ScripterTree extends AbstractMutableRefTree {
    ...
    private List /*!5 <Edit> */ edits;
    private MutableRefTree t;

    public ScripterTree(List /*!5 <Edit> */ edits, MutableRefTree t) {
      this.edits = edits;
      this.t = t;
    }
  ...
  private static void insertTree(MutableRefTree t, Key pid, RefTreeNode r,
    ...
    Key nk = t.insert(pid, autoKey ? MutableRefTree.AUTO_KEY :  r.getId(),
        r.getContent());
    for( Iterator /*!5 <RefTreeNode> */ i = r.getChildIterator();i.hasNext();)
      insertTree(t,nk, (RefTreeNode) i.next(), autoKey);
  }
}
