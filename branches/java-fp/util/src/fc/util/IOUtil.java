...
public class IOUtil {
    ...
    private static LinkedList/*!5 <byte[]>*/ buffers = 
	new LinkedList /*!5 <byte[]>*/();
	...
	//!A assert canMoveOver != null;
//	if( canMoveOver.booleanValue() && !tmpExists ) 
//	    Log.debug("Creating (for move over)",tmp);
	if( canMoveOver.booleanValue() && !tmpExists && !tmp.createNewFile() ) {
	    throw new IOException("Cannot create "+tmp);
	} else if ( canMoveOver.booleanValue() && tmpExists ) {
	...
	if( !f.renameTo(tmp) ) {
	    if( canMoveOver.booleanValue() && !tmp.delete() )
		    throw new IOException("Cannot delete "+tmp);		
	    throw new IOException("Cannot replace "+f+" with "+tmp+
		    (canMoveOver.booleanValue() ? "(by move over)" : ""));
	}
	return tmp;
    }
...
//		if( canMoveOver.booleanValue() )
//		    Log.debug("Renaming (move over cleanup) "+tmp2+" to "
//			    +origTmp);
		if( canMoveOver.booleanValue() && !tmp2.renameTo(origTmp) ) {
