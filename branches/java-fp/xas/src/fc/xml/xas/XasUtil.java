...
public class XasUtil {
    ...
    public static final Qname BASE64_BINARY_TYPE =
	new Qname(XSD_NS, "base64Binary");
	...
	FormatFactory existing = (FormatFactory) factories.get(type);
	if (existing == null) {
	    factories.put(type, factory);
	} else if (!existing.getClass().equals(factory.getClass())) {
	    throw new IllegalArgumentException("Type " + type
		+ " already reserved for factory class " + existing.getClass());
	}
    }

    public static FormatFactory getFactory (String type) {
	return (FormatFactory) factories.get(type);
    }

    public static Collection factoryTypes () {
	return factories.keySet();
    }

    public static void copy (ItemSource source, ItemTarget target)
	    throws IOException {
	copy(source, target, Long.MAX_VALUE);
    }

    public static void copy (ItemSource source, ItemTarget target, long max)
	    throws IOException {
	Item item;
    ...
    public static void copy (ItemSource source, List<Item> target)
	    throws IOException {
	Item item;
    ...
    public static void copyFragment (ItemSource source, ItemTarget target)
	    throws IOException {
	Item item;
	while ((item = source.next()) != null) {
    ...
    public static void copyFragment (ItemSource source, List<Item> target)
	    throws IOException {
	Item item;
	while ((item = source.next()) != null) {
	    if (!Item.isDocumentDelimiter(item)) {
    ...
    /**
         * Return non-fragment item for this item. The method returns the first
         * item that is not a fragment by removing any fragment item
         * encapsulations. I.e.
         * <code>skipFragment( XF(XF(ST({}tag))) ) = ST({}tag)</code>.
         * 
         * @param i item to strip fragments from
	...
	public Item next () throws IOException {
	    if (checkReads && left <= 0)
		throw new IOException("Read past end of source");
    ...
    /**
         * Item source that returns only non-fragment items. Auto-forces any
         * lazy fragments.
         * 
         */
	...
	public Item next () throws IOException {
		...
		if (i instanceof LazyFragment)
		    ((LazyFragment) i).force(1);
		fiStack.push(fi);
		posStack.push(new Integer(pos));
		pos = 0;
		fi = (FragmentItem) i;
		return next();
	    } else if (i == null) {
		if (fiStack.isEmpty())
		    return null;
		fi = (FragmentItem) fiStack.pop();
		pos = ((Integer) posStack.pop()).intValue();
		return next();
	    }
	    return i;
	}
    ...
    /**
         * Item source on top of an array of items. Supports starting from a
         * specific offset and ending at a specific offset.
         */
