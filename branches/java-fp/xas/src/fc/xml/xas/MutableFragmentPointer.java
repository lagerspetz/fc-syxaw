...
public class MutableFragmentPointer extends FragmentPointer implements
	MutablePointer {
	...
	if (!xasFragment.convert(index)) {
	    throw new IllegalStateException("Pointer not at StartTag");
	}
	...
	if (Item.isStartTag(result)) {
	    if (!xasFragment.convert(index)) {
		throw new IllegalStateException("Conversion of item " + result
			+ " failed");
	    }
	    result = get();
	...
	if (Item.isStartTag(item)) {
	    if (!xasFragment.convert(index)) {
		throw new IllegalStateException("Conversion of item " + item
			+ " failed");
	    }
	    item = get();
	}
	...
	return xasFragment.query(
	    new MutableFragmentPointer(xasFragment, index), path);
    }
