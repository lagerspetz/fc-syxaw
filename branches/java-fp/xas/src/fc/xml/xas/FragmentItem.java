...
/**
 * An item representing a sequence of items. A fragment has two size-like
 * properties. Its length is the number of items that it contains. If it is
 * contained in another fragment, its size is the number of items that it
 * contains in its parent fragment. Therefore the expression
 * <code>i + f.getSize()</code> for a fragment <code>f</code> at index
 * <code>i</code> gives the index of the item following <code>f</code>.
 */
public abstract class FragmentItem extends Item implements AppendableItem {
	...
	if (index == 0) {
	    if (firstItem == null) {
		throw new IllegalArgumentException("Fragment does not have "
			+ "item 0");
	    }
	} else if (items == null) {
	...
	if (items != null) {
	    int n = items.size();
	    for (int i = 1; i < n; i++) {
		target.append((Item) items.get(i));
	    }
	}
    }

    /*
         * XXX - the following two methods will disappear, possibly in place of
         * this class implementing the List<Item> interface in some manner
         */
    public List getFragmentContent () {
	List result = null;
	if (Item.isStartItem(firstItem)) {
		...
		if (Item.isStartTag(item)) {
		    ...
		    while (depth > 0) {
			if (i >= n) {
			    throw new IllegalStateException("An unbalanced "
				    + "fragment: " + items);
			}
		    ...
		    Item it = (Item) items.get(i + 1);
		    if (Item.isContent(it)) {
			List fragment = new ArrayList();
			fragment.add(item);
			while (Item.isContent(it)) {
			    fragment.add(it);
			    it = (Item) items.get( ++i + 1);
			}
			result.add(new XasFragment(fragment, item));
		    } else {
	...
	if (this.items != null && this.items.size() > 0) {
	    Item lastItem = (Item) this.items.get(this.items.size() - 1);
	    this.items.clear();
