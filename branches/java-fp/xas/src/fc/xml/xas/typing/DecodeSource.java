...
public class DecodeSource implements ItemSource {
	...
	if (Log.isEnabled(Log.DEBUG)) {
	    Log.debug("Stack state, top = " + stackTop, tagStack);
	    Log.debug("Buffer state, index=" + index + ",end=" + end, buffer);
	}
	int top = pop();
	...
	if (index < end) {
	    ...
	    if (Log.isEnabled(Log.DEBUG)) {
		Log.debug("Returning buffered item", item);
	    }
	    return item;
	} else {
	    ...
	    if (Log.isEnabled(Log.DEBUG)) {
		Log.debug("Returning normal item", item);
	    }
	    return item;
	}
    }
