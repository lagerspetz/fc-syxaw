 ...
 * <code>ItemTarget</code> and implements the SAX {@link ContentHandler}
 ...
 * code. New classes are recommended to use the {@link ItemTarget} interface
 * directly and its implementing classes to handle XML.
 */
public class SaxToItems implements ContentHandler, DTDHandler, LexicalHandler,
	DeclHandler {
	...
	if (target != null) {
		...
		/*NOTFP if( target instanceof Flushable )
	...
	/* FIXME-20061017-1: how do we handle prefixes?
	if (target != null) {
	    try {
	...
	if (target != null) {
		...
		if (attributes != null) {
			...
			st.addAttribute(new Qname(attUri, localAttName),
