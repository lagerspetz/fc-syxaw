...
/**
 * A class for iterating over a stream of items while maintaining a key. In
 * indexing applications it is often necessary to iterate over an item stream
 * and to mark the Dewey keys of (some) encountered items somehow. This class
 * handles the maintenance of the key during the iteration, so the application
 * code only needs to contain the application logic.
 */
public class KeyIterator {
    ...
    /**
         ...
         * 
         * @param item the current item from the processed stream
         * 
         * @throws IllegalStateException if <code>item</code> is a document
         *         delimiter and the current key is not root outside the
         *         delimiter
         */
