...
/** AWT Tabbed panel. Original by Pete Yared, modified by Jean-Guy Speton;
 * read the sources.
 */
...
public class TabPanel extends Panel implements MouseListener
{
  ...
  protected Font tabFont = new Font( "Dialog", Font.PLAIN, 12);
  ...
  public void removePanel(String label)
  {
    ...
    if (index == -1) {
      throw new IllegalArgumentException(label +
					 " is not a panel in this TabPanel");
    }
  ...
  public void paint(Graphics g)
  {
    ...
    if (selected > -1) {
      paintTab(g, true, selectedLoc, 0, tabWidths[selected], top,
	       (String) labels.elementAt(selected));
      g.setColor(Color.white);
			...
			width, int height, String label)
  {
    ...
    g.drawString(label, x + ((width - metrics.stringWidth(label)) / 2),
		 metrics.getHeight() + top + 1);
...
// arch-tag: base-uri-http://www2.blaze.ca/~jspeton/java/tabpanel/TabPanel.java
